...
function nativeRoleId(channel: string, role: string) {
  return role === "series" ? "series" : channel;
}
...
export function analyzeChartSpecRepairs(dataset: Dataset, spec: ChartSpec): ChartRepairAnalysis {
  ...
  const roles: ChartRepairRoleContract[] = template.channels.map((channel) => ({
    id: nativeRoleId(channel.channel, channel.role),
    kind: channel.role === "dimension" || channel.role === "series" ? "dimension" : channel.role,
    accepts: channel.accepts,
    minFields: channel.required ? 1 : 0,
    maxFields: 1,
    requiresPartition: channel.role === "dimension" || channel.role === "series",
  ...
  template.channels.forEach((channel) => {
    const roleId = nativeRoleId(channel.channel, channel.role);
    const encoding = channel.role === "series"
      ? spec.series ?? spec.encodings[channel.channel]
      : spec.encodings[channel.channel];
    if (encoding) binding[roleId] = [encoding.field];
  });
