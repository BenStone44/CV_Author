...
function radialClusterTemplateSvg() {
  ...
  return frame(`<g transform="translate(${cx} ${cy})" data-renderer="observable-radial-cluster@3" data-angle-span="360" data-leaf-radius="${leafRadius}" data-selection-radius="${selectionRadius}"><g fill="none" stroke="#555" stroke-opacity="0.4" stroke-width="2.5">${links}</g><g>${marks}</g><g stroke-linejoin="round" stroke-width="3" paint-order="stroke" stroke="white" fill="currentColor" font-size="8">${labels}</g></g>`);
}

function boxplotTemplateSvg(multiple: boolean) {
  const positions = multiple ? [62, 126, 194, 258] : [166];
  const marks = positions.map((x, index) => {
    const color = advancedPalette[index % advancedPalette.length]!;
    const top = 36 + index % 2 * 9;
    const bottom = 140 - index % 3 * 7;
    const q3 = 58 + index % 2 * 8;
    const q1 = 112 - index % 3 * 6;
    const median = 82 + index % 3 * 5;
    return `<g data-mark-role="box" data-category-key="${multiple ? `Group ${index + 1}` : "All"}"><path d="M${x} ${top}V${bottom}" stroke="${color}" stroke-width="1.5"/><rect x="${x - 20}" y="${q3}" width="40" height="${q1 - q3}" fill="${color}" fill-opacity=".72" stroke="${color}"/><path d="M${x - 20} ${median}H${x + 20}" stroke="${color}" stroke-width="2.5"/></g>`;
  }).join("");
  return frame(`${axis}<g data-boxplot-mode="${multiple ? "multiple" : "single"}" data-palette="global">${marks}</g>`);
}

const areaPreviewPoints = [
  [28, 125], [48, 117], [68, 113], [88, 119], [108, 116],
  [128, 91], [148, 98], [168, 94], [188, 66], [208, 73],
  [228, 67], [248, 39], [268, 46], [288, 43], [304, 56],
] as const;
const areaPreviewLine = areaPreviewPoints.map(([x, y], index) => `${index === 0 ? "M" : "L"}${x} ${y}`).join("");
const areaPreviewDots = areaPreviewPoints.map(([x, y]) => `<circle cx="${x}" cy="${y}" r="2.5"/>`).join("");

export const advancedTemplateSvgs = {
  AreaChart: frame(`${axis}<path d="${areaPreviewLine}L304 152L28 152Z" fill="#4f8fc4" fill-opacity=".8" stroke="#2563eb" stroke-width="1.5"/><g fill="#fff" stroke="#2563eb" stroke-width="1.8">${areaPreviewDots}</g>`),
  ...
  Icicle: frame(`<g fill-opacity="0.68"><rect x="8" y="5" width="48" height="170" fill="${advancedPalette[0]!}"/><rect x="57" y="5" width="67" height="76" fill="${advancedPalette[1]!}"/><rect x="57" y="82" width="67" height="93" fill="${advancedPalette[2]!}"/><rect x="125" y="5" width="91" height="38" fill="${advancedPalette[3]!}"/><rect x="125" y="44" width="91" height="37" fill="${advancedPalette[4]!}"/><rect x="125" y="82" width="91" height="45" fill="${advancedPalette[5]!}"/><rect x="125" y="128" width="91" height="47" fill="${advancedPalette[6]!}"/><rect x="217" y="5" width="95" height="20" fill="${advancedPalette[7]!}"/><rect x="217" y="26" width="95" height="17" fill="${advancedPalette[8]!}"/><rect x="217" y="44" width="95" height="37" fill="${advancedPalette[0]!}"/><rect x="217" y="82" width="95" height="23" fill="${advancedPalette[1]!}"/><rect x="217" y="106" width="95" height="21" fill="${advancedPalette[2]!}"/><rect x="217" y="128" width="95" height="47" fill="${advancedPalette[3]!}"/></g><g font-size="7"><text x="12" y="16">root</text><text x="61" y="16">analytics</text><text x="61" y="94">visualization</text></g>`),
  ...
  RadialStackedBarChart: frame(""),
  RadialRectBarChart: frame(""),
  RadialRectStackedBarChart: frame(""),
  CircularBarChart: frame(""),
  CircularStackedBarChart: frame(""),
  RadarChart: frame(""),
  ...
  SingleBoxplot: () => boxplotTemplateSvg(false),
...
const definitions: Array<[keyof typeof advancedTemplateSvgs, string, string, SvgCandidate["coordinateSystem"]]> = [
  ...
  ["RadialBarChart", "Radial Bar (Sector)", "RadialBarChart", "Polar"],
  ["RadialStackedBarChart", "Radial Stacked Bar (Sector)", "RadialStackedBarChart", "Polar"],
  ["RadialRectBarChart", "Radial Bar (Rectangle)", "RadialRectBarChart", "Polar"],
  ["RadialRectStackedBarChart", "Radial Stacked Bar (Rectangle)", "RadialRectStackedBarChart", "Polar"],
  ["CircularBarChart", "Circular Bar", "CircularBarChart", "Polar"],
  ["CircularStackedBarChart", "Circular Stacked Bar", "CircularStackedBarChart", "Polar"],
  ["RadarChart", "Radar Chart", "RadarChart", "Polar"],
  ["Calendar", "Calendar", "Calendar", "CoordinateFree"],
  ["SingleBoxplot", "Single Box Plot", "SingleBoxplot", "Cartesian"],
