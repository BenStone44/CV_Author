...
Before publishing, inspect the complete production output for personal identifiers, source-repository references, metadata, and source maps, while retaining required third-party attribution. Build outside `dist/`, preserve existing public history, and verify the staged site and Gallery/editor entry modes before pushing. Post-push production verification is user-owned; until the user completes it, report the release as published and awaiting verification rather than verified. See the root `AGENTS.md` for the full anonymity and release checklist.

Latest published release awaiting user verification: `2026-09-16`, public deployment commit `c4b0583c78366f2b01d7d9ee83c2064e280dd5d9` (`Publish on-demand editor data loading`). All automated pre-push gates passed before the exact reviewed commit was pushed, including all 16 prepared starters and all 16 completed cases. No automated post-push browser or byte-comparison checks are part of the publishing command.

Latest verified release: `2026-09-15`, public deployment commit `11161403630971581423a3bcc3304cdc814dac3e`. Publication used the repository-scoped deploy key and project-only author/committer identity, preserving public history. GitHub Pages workflow `34937923699` succeeded. Staged and live checks passed for all website entries including the API reference, all six prepared starters, and all six completed cases; all 169 non-hidden public files matched the staged release bytes, and eleven obsolete hashed assets were confirmed removed. The user approved retaining the existing Mapbox account identifiers in its style URLs/public token; all other scoped identity checks passed. See `AGENTS.md` for the full verification record.
...
The command refuses a dirty source tree, an unexpected deploy key or public commit identity, stale third-party notices, identity-bearing build output, source maps, unreviewed stale public files, build/type errors, or failed staged browser checks. Before pushing, it preserves public history, normalizes Gallery capture metadata, backs up obsolete hashed assets outside the Pages checkout, and verifies all Gallery starters and completed cases against the isolated production build. After a successful push it prints the release commit and exits. It intentionally does not wait for GitHub Pages, request the production site, or compare live files; those post-push checks belong to the user. The detailed gates below document what the command enforces and remain the recovery/reference procedure.
...
#### 5. User-owned production verification and release record

The automated publishing command stops after the reviewed commit is pushed. The user owns every check in this section; agents and release automation must not run them unless the user explicitly requests it. Until the user confirms them, record the outcome as **published, awaiting user verification**, not as a verified production release.

The user may wait for GitHub's `pages build and deployment` workflow to report success for the exact release commit, using the repository's Actions page or public API at `https://api.github.com/repos/VisBricks/visbricks.github.io/actions/runs?per_page=5`. A successful push alone does not establish that the production deployment was verified. If the workflow or live checks fail, retain the status as pushed but not verified; do not silently roll back or report completion.
...
- `/editor/?starter=<slug>` for every Gallery entry in `/site/gallery/cases/index.json`: matching starter identity, `data-starter-status="ready"`, and independent blocks with their intended bindings. Verify all 16 current entries, including `geographic-network` and `tree-leaf-axis`.
- `/editor/?case=<case-id>` using each manifest entry’s `completedCase`: matching `data-case-id`, `data-case-status="ready"`, and valid SVG in `window.__VISBRICKS_CASE_SVG__`. Verify all 16 current completed cases, including Geographic Network and Dendrogram Profiles.
